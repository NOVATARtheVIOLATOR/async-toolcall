...
If anyone is interested in making a Codex MCP server, app, plugin, proxy, or controller around this and needs some assistance or information, feel free to contact me by DM. I'll explain how it works in more detail.

# Demonstration

I will demonstrate the async tool method here so you can see what it is like.

If you're a software developer, you'll probably find the basic proof of concept relatively simple overall. I'm sure it can be vibe coded too, though I'd recommend a more advanced approach if you want it to work reliably outside a toy demo.

There are a variety of ways to do this, and I'm not going to post all the specs here, but I will describe the overall process.

In order for this to work, a couple of things need to be done.

# API call side

You must both set a Developer prompt, or instructions field, **and** define your tools a certain way.

In the end, it is clever prompt/context engineering, though the API call is only part of the solution.

Below is a very simple Developer prompt as a proof of concept.

The proof-of-concept script is very simple and crude, but it demonstrates the overall idea. It makes the API treat tool responses as events to respond to, even if they occur on their own during regular conversation, while the actual tool call is separated from them by multiple turns.

The Developer prompt instructs the agent to allow tool calls to be separated from their tool responses, allowing normal conversation to continue.

This one is all-encompassing and does not necessarily have to target all tools, but it can be done that way if you want to make all tools asynchronous.

If you do that, you would need to intercept all MCP server requests. It will not make them interactive by itself, because that requires the tool itself to be custom made, but it can allow long-running tools to get out of the way.

If you want to apply it to all tools, you will have to carefully teach the agent to deal with tools in a new way, which can have issues.
...
It has to be able to keep a tool/MCP connection open, respond to incoming message events, send outgoing messages when needed, maintain keepalive on the connection, and automatically reconnect.

As long as this channel is open, it will allow async bidirectional model calls and responses to occur as needed.
